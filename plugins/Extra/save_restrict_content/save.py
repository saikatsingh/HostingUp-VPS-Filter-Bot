...
async def get_msg(client, sender, edit_id, msg_link, i):
    ...
    if ('t.me/c/' in msg_link) or ('t.me/b/' in msg_link):
        ...
        file = ""
        try:
            amrobots_tech_userbot = Client("saverestricted", session_string=SESSION_STRING, api_hash=API_HASH, api_id=API_ID)
            await amrobots_tech_userbot.start()
            msg = await amrobots_tech_userbot.get_messages(chat, msg_id)
            if not msg.media:
                if msg.text:
            ...
            file = await amrobots_tech_userbot.download_media(
                msg,
            ...
            if msg.media==MessageMediaType.VIDEO_NOTE:
                ...
                try:
                    thumb_path = await amrobots_tech_userbot.download_media(msg.video_note.thumbs[0].file_id)
                except Exception:
                    thumb_path = None
                ...
                print(f'd: {duration}, w: {width}, h:{height}')
                try:
                    thumb_path = await amrobots_tech_userbot.download_media(msg.video.thumbs[0].file_id)
                except Exception:
                    thumb_path = None
                ...
                try:
                    thumb_path = await amrobots_tech_userbot.download_media(msg.document.thumbs[0].file_id)
                except Exception:
                    thumb_path = None
...
async def progress_for_pyrogram(
    ...
    if round(diff % 10.00) == 0 or current == total:
        ...
        tmp = progress + "⏳ **ɢʀᴏss:** **{0}** **ᴏғ** **{1}**\n\n🚀 **sᴘᴇᴇᴅ:** **{2}**/**s**\n\n🕛 **ᴇᴛᴀ:** **{3}**\n\n**Powered By : @AmRobots_Bots**".format(
            humanbytes(current),
            humanbytes(total),
            humanbytes(speed),
